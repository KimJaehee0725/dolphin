...
- ignored `config/research-memory.env` overlay의 `RESEARCH_MEMORY_ROOT` 한 값이
  private host-side research-memory bundle을 가리킨다.
...
- 에이전트는 각 컨테이너 세션의 첫 substantive task에서 접근을 확인하고,
  미설정이면 사용자 동의 후 host-side enable helper로만 설정을 돕는다.
